...
engine = create_engine('postgresql+psycopg2://postgres:egorbro26@localhost/database1')
...
while server_works:
    ...
    for i in range(0, len(pairs)):
        for j in range(i+1, len(pairs)):
            ...
            if abs(dist_x) <= hero_1.w_vison // 2 + hero_2.size and abs(dist_y) <= hero_1.h_vision // 2 + hero_2.size:
                x_ = str(round(dist_x))
                y_ = str(round(dist_y))
            ...
            if abs(dist_x) <= hero_2.w_vison // 2 + hero_1.size and abs(dist_y) <= hero_2.h_vision // 2 + hero_1.size:
                x_ = str(round(-dist_x))
                y_ = str(round(-dist_y))
